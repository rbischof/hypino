...
class CSGUnion(Geometry):
    ...
    def on_boundary(self, x, atol=1e-6):
        return np.logical_or(
            np.logical_and(self.geom1.on_boundary(x, atol=atol), ~self.geom2.inside(x)),
            np.logical_and(self.geom2.on_boundary(x, atol=atol), ~self.geom1.inside(x)),
        )
...
class CSGDifference(Geometry):
    ...
    def on_boundary(self, x, atol=1e-6):
        return np.logical_or(
            np.logical_and(self.geom1.on_boundary(x, atol=atol), ~self.geom2.inside(x)),
            np.logical_and(self.geom1.inside(x), self.geom2.on_boundary(x, atol=atol)),
        )
