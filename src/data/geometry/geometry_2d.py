...
class Disk(Geometry):
    ...
    def on_boundary(self, x, atol=1e-6):
        return np.isclose(np.linalg.norm(x - self.center, axis=-1), self.radius, atol=atol)
...
class Triangle(Geometry):
    ...
    def boundary_normal(self, x):
        ...
        on_vertices = np.count_nonzero(np.hstack([on12, on23, on31]), axis=-1) > 1
        on12[on_vertices] = 0
        on23[on_vertices] = 0
        on31[on_vertices] = 0
...
class Polygon(Geometry):
    ...
    def on_boundary(self, x, atol=1e-6):
        _on = np.zeros(shape=len(x), dtype=np.int32)
        for i in range(-1, self.nvertices - 1):
            l1 = np.linalg.norm(self.vertices[i] - x, axis=-1)
            l2 = np.linalg.norm(self.vertices[i + 1] - x, axis=-1)
            _on[np.isclose(l1 + l2, self.diagonals[i, i + 1], atol=atol)] += 1
        return _on > 0

    def boundary_normal(self, x):
        n_points = x.shape[0]  # Number of points
        result_normals = np.zeros((n_points, 2))

        for i in range(self.nvertices):
            on_segment = is_on_line_segment(self.vertices[i - 1], self.vertices[i], x)
            result_normals[on_segment] = self.normal[i]
        return result_normals
    
    def boundary_normal(self, x):
        """Compute normal vectors on the boundary for an array of points.
        
        Args:
            vertices: Array of vertices defining the polygon.
            normals: Array of normal vectors for each polygon edge.
            x: An array of points for which to compute the boundary normals (B, D).
        
        Returns:
            An array of normal vectors for each point in `x` that lies on the boundary.
        """
        n_points = x.shape[0]  # Number of points
        result_normals = np.zeros((n_points, 2))
        
        for i in range(self.nvertices):  # Iterate over each edge of the polygon
            P0 = self.vertices[i - 1]
            P1 = self.vertices[i]
            on_segment = is_on_line_segment(P0, P1, x)
            result_normals[on_segment] = self.normal[i]
...
def is_on_line_segment(P0, P1, P2):
    ...
    return np.logical_and(
        ...
        np.isclose(np.cross(v01, v02, axis=-1) / np.linalg.norm(v01), 0, atol=1e-6),
        # check that projection of P2 to line is between P0 and P1
        (v01 * v02).sum(axis=-1) >= 0,
        (v01 * v12).sum(axis=-1) <= 0,
    )
